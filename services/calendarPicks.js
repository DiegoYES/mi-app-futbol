...
function incluyeEquipo(partido, equipoId) {
  return partido.equipo_local?.id === equipoId || partido.equipo_visitante?.id === equipoId;
}
...
async function analizarPartidosCalendario(partidos, limiteMuestra = 10) {
  ...
  for (const partido of analizables) {
    ...
    if (!grupos.has(clave)) grupos.set(clave, { liga: partido.liga.id, temporada: partido.liga.temporada, equipos: new Set(), hasta: partido.fecha });
    const grupo = grupos.get(clave);
    grupo.equipos.add(partido.equipo_local.id);
    grupo.equipos.add(partido.equipo_visitante.id);
  ...
  await Promise.all([...grupos.entries()].map(async ([clave, grupo]) => {
    ...
    const historicos = await Partido.find({
      ...
      $or: [{ 'equipo_local.id': { $in: equipos } }, { 'equipo_visitante.id': { $in: equipos } }]
    }).sort({ fecha: -1 }).lean();
  ...
  return analizables.map(partido => {
    ...
    const resultado = generarPicks({
      partidosLocal: historicos.filter(item => incluyeEquipo(item, partido.equipo_local.id)),
      teamLocal: partido.equipo_local.id,
      partidosVisitante: historicos.filter(item => incluyeEquipo(item, partido.equipo_visitante.id)),
